...
#pragma omp critical
    ...
    MPI_win_create(vecteurs, m * n * sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &wint_t);

    // syncroœnisation pour que tous les processus aient accès à la fenêtre
    MPI_Win_fence(0, wint_t);
    (pid != root){
        MPI_Get(vecteurs, m * n, MPI_INT, root, 0, m * n, MPI_INT, wint_t);
    }
    MPI_Win_fence(0, wint_t);

    int vect_per_proc = m / nprocs;  // nombre de vecteurs par processus
    int *local_vecteurs = new int[vect_per_proc * n];
    int *local_resultats = new int[vect_per_proc * n];

    // creation d'une fenetre pour la matrice
    MPI_Win wint_m;
    if(pid == root){
        MPI_win_create(matrice, n * n * sizeof(int), sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &wint_m);
    }
    else{
        MPI_win_create(NULL, 0, sizeof(int), MPI_INFO_NULL, MPI_COMM_WORLD, &wint_m);
    }

    //chaque processus récupère sa portion de vecteur  remplace (MPI _scatter)
    MPI_Win_fence(0, wint_m);
    if(pid != root){
        MPI_Get(matrice, n * n, MPI_INT, root, 0, n * n, MPI_INT, wint_m);
    }
    MPI_Win_fence(0, wint_m);
